...
public class MyHeap<E extends Comparable> implements HeapInterface<E> {
    ...
    public boolean insert(E item) { //todo -write
        int positionOfNewAdd = internalHeap.size() - 1;
        ...
        if (internalHeap.get(positionOfNewAdd).compareTo(internalHeap.get(parentIndex(positionOfNewAdd))) >= 1) { //if parent is bigger, heapify-up
            do {
                ...
                internalHeap.set(parentIndex(positionOfNewAdd), internalHeap.get(positionOfNewAdd));
                internalHeap.set(parentIndex(positionOfNewAdd), temp);
                positionOfNewAdd = parentIndex(positionOfNewAdd); //adjust the position of the newly added element to it's parent.
            } while (positionOfNewAdd != 0 || internalHeap.get(parentIndex(positionOfNewAdd)).compareTo(internalHeap.get(positionOfNewAdd)) <= -1); //continue until at root or parent is smaller
        }
        return true;
    }
